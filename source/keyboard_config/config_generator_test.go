...
func Test_ConvertLayerToErgodoxPrexyAsString(t *testing.T) {
...
KC_LEFT_CTRL, KC_DELETE, KC_LEFT_SHIFT, KC_LEFT_GUI, KC_LEFT_ALT, KC_TAB, KC_BACKSPACE, KC_SPACE
	...
	actualValue := ConvertLayerToErgodoxPrexyAsString(keyboardFullValidationSet)
	expected := strings.ReplaceAll(strings.Trim(expected_layout_ergodox_pretty, cutSet), "\n", "")
	actual := strings.ReplaceAll(strings.Trim(actualValue, cutSet), "\n", "")
...
`
	...
	arrayToIterate := arrayFirstPass[1 : len(arrayFirstPass)-1]
	actual := strings.Join(arrayToIterate, "\n")
	for index, eachValueExpectedRune := range actual {
