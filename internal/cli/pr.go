...
func createPRForRepo(repo RepoConfig, branchName string) (string, error) {
	...
	// Clone the target repository (full clone to get all branches)
	if verbose {
		fmt.Printf("   📥 Cloning %s...\n", repo.RemoteURL)
	}
	cloneCmd := exec.Command("gh", "repo", "clone", repo.RemoteURL, tempDir)
	...
	defer os.Chdir(klcmDir)

	// Get the target branch for this repo
	targetBranch := getTargetBranch(repo)
	
	// Try to checkout the target branch, fall back to default if needed
	if err := runGitCommandQuiet("checkout", targetBranch); err != nil {
		...
		for _, line := range lines {
			if strings.Contains(line, "HEAD branch:") {
				...
				if len(parts) > 1 {
					targetBranch = strings.TrimSpace(parts[1])
					break
				}
			}
		}
	...
	if err := runGitCommandQuiet("checkout", "-b", branchName); err != nil {
		return "", fmt.Errorf("failed to create branch: %w", err)
	}

	// Copy the local config file(s) to the cloned repo
	localConfigPath := filepath.Join(klcmDir, repo.LocalPath, repo.DefaultFile)
	targetConfigPath := getTargetConfigPath(repo, tempDir)
	
	if verbose {
		fmt.Printf("   📋 Copying %s -> %s\n", localConfigPath, targetConfigPath)
	}
	...
	// Write to target
	if err := os.WriteFile(targetConfigPath, content, 0644); err != nil {
	...
	if err != nil || strings.TrimSpace(string(output)) == "" {
		return "", fmt.Errorf("no changes to commit - local and remote files are identical")
	}

	// Add and commit
	if err := runGitCommandQuiet("add", "."); err != nil {
		return "", fmt.Errorf("failed to add changes: %w", err)
	}

	commitMsg := fmt.Sprintf("Update %s keyboard layout configuration\n\nGenerated by KLCM (Keyboard Layout Configuration Mapper)\n\nChanges include:\n- Synced keymap from KLCM source of truth\n- Added mapping layer for translation support\n- Added macros and mod-morph behaviors", repo.Name)
	
	if err := runGitCommandQuiet("commit", "-m", commitMsg); err != nil {
...
- Added mapping layer for cross-keyboard translation
- Added macros (brackets, braces, parens, angle_brackets)
- Added mod-morph behaviors (dot→colon, comma→semicolon, etc.)
...
func runGitCommandQuiet(args ...string) error {
	cmd := exec.Command("git", args...)
	return cmd.Run()
}
...
// getTargetBranch returns the target branch for a repo
func getTargetBranch(repo RepoConfig) string {
	switch repo.Name {
	case "adv_mod":
		return "cheyo"
	default:
...
// getTargetConfigPath returns the path where the config file should be placed in the target repo
func getTargetConfigPath(repo RepoConfig, repoDir string) string {
	switch repo.Name {
	case "adv360":
		return filepath.Join(repoDir, "config", "adv360.keymap")
	case "glove80":
		return filepath.Join(repoDir, "config", "glove80.keymap")
	case "adv_mod":
		return filepath.Join(repoDir, "config", "adv_mod.keymap")
	default:
		return filepath.Join(repoDir, "config", repo.DefaultFile)
	}
}
